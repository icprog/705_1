...
struct rt_device dev_mma8451;

/*是否是首次运行，首次运行不上报状态*/
static uint8_t mma845_print_style	= 0;		//1.表示侧翻, 2.侧翻回正, 3.表示碰撞
int 		alarm_angle	= 60;
static const char *mma845_print_str[]=
{
	"\n侧翻",
	"\n侧翻回正",
	"\n碰撞触发!"
...
* Others:         // 其它说明
***********************************************************/
static uint8_t IIC_RegReadData( uint8_t address,uint8_t reg, uint8_t *pdata, uint8_t count )
{
	uint8_t err				= ERR_NONE;
...
uint16_t			param_mma8451_word2 = 0x1E30; /*0x1e:单位4ms的倍数  0x30:碰撞加速度门限 0.1g的倍数*/
...
#define TILT_ON		1
#define TILT_OFF	2
static uint32_t collide_tick= 0;			/*碰撞发生的时刻*/
...
void mma845_read(void)
{
 rt_device_read(&dev_mma8451, 0, RT_NULL, 0);
}
FINSH_FUNCTION_EXPORT( mma845_read, setup sensor );
...
static uint8_t mma8451_config( uint16_t param1, uint16_t param2 )
{
	...
	value = jt808_param.id_0x005D & 0xFF;
	if(value < 10)
		value = 10;
	else if(value > 79)
		value = 79;
	param_mma8451_word1 = param1;
	param_mma8451_word2 = param2 & 0xFF00;
	param_mma8451_word2 += value;
 ...
 */
 	///设置量程为8G
	if( IIC_RegWrite( MMA845X_ADDR, XYZ_DATA_CFG_REG, FULL_SCALE_8G ) )
	{
		goto lbl_mma8451_config_err;
	}
 ...
 */
	...
	///加速度门限 0.1g 1-79	扩展为0.0625  1-127  扩大1.6倍
	///设置碰撞门限，
	res = ( param_mma8451_word2 & 0xff ) * 16 / 10;
	...
	///Data Rate 200 Hz, time step is 2.5 ms
	///600 ms/2.5 ms = 240 counts
	if( IIC_RegWrite( MMA845X_ADDR, PULSE_LTCY_REG, 240 ) )
	{
		goto lbl_mma8451_config_err;
	}
...
{
 alarm_angle = angle;
 rt_kprintf("\n%d>set angle= %d",rt_tick_get( ),angle);
}
FINSH_FUNCTION_EXPORT( set_angle, set_angle );
...
* Others:         // 其它说明
***********************************************************/
static rt_size_t read_acc( void )
{
	u8 temp1[8];
	s16 x,y,z;
	double dx,dy,dz;
	...
	err=IIC_RegReadData(MMA845X_ADDR,OUT_X_MSB_REG,temp1,6);
	if(err==ERR_NONE)
		{
		 x = ((u16)temp1[0]<<8)+temp1[1];
		 x /=4;
		 y = ((u16)temp1[2]<<8)+temp1[3];
		 y/=4;
		 z = ((u16)temp1[4]<<8)+temp1[5];
		 z/=4;
		 dx=abs(x);
		 dy=abs(y);
		 dz=abs(z);
		 ...
		 if(angle>alarm_angle)
			...
			if(AlarmCounter>=3)
				{
				jt808_param_bk.car_alarm |= ( BIT_ALARM_TILT );
				if(AlarmCounter == 3)
					{
					rt_kprintf("\n 车辆侧翻!");
					}
		 ...
		 //rt_kprintf("\r\n  mma8451_read     x=%d,y=%d,z=%d , angle=%d",x,y,z,angle);
		}
	else
		{
		rt_kprintf("\r\n mma8451_read_Error!");
...
static rt_size_t mma8451_read( rt_device_t dev, rt_off_t pos, void* buff, rt_size_t count )
{
	...
	if( !GPIO_ReadInputDataBit( GPIOD, GPIO_Pin_5 ) )
	{
		...
		//rt_kprintf("\n INT_SOURCE_REG=%02x, PL_STATUS_REG=%02x, PULSE_SRC_REG=%02x,",value1,value2,value3);
		//if((value2&0x7F)<0x40)	/*有倾斜发生  0x8x 倾斜发生*/
		if(( value1 & 0x10 )&& (value2 < 0xc0) ) 					/*翻了*/
		{
			//beep( 2, 1, 1 );
		...
		if((value1 & 0x08)&&value3)
		{
			mma845_print_style	= 3;
			collide_tick = rt_tick_get( );	/*记录当前碰撞触发时刻*/
			jt808_param_bk.car_alarm |= ( BIT_ALARM_COLLIDE );
		}
		//rt_kprintf( "\nINT=%02x %02x\n", value2, value3 );
	}
	else
...
static void cb_tmr_sensor( void* parameter )
{
	...
	if( tilt_status == TILT_ON )
	{
		...
		{
			if( ( jt808_param_bk.car_alarm & BIT_ALARM_TILT ) == 0 )
			{
				//rt_kprintf( "\n侧翻" );
				mma845_print_style	= 1;
			}
			jt808_param_bk.car_alarm |= ( BIT_ALARM_TILT );
		}
	}
	if( tilt_status == TILT_OFF )
	{
		if( rt_tick_get( ) - tilt_tick > 300 ) /*3秒*/
		{
			if( jt808_param_bk.car_alarm & BIT_ALARM_TILT )
			{
				//rt_kprintf( "\n侧翻回正" );
				mma845_print_style	= 2;
			}
			jt808_param_bk.car_alarm &= ~( BIT_ALARM_TILT );
	...
	///碰撞操作
	if( jt808_param_bk.car_alarm & BIT_ALARM_COLLIDE )
	{
		if( rt_tick_get( ) - collide_tick > 500 )
		{
			jt808_param_bk.car_alarm &= ~(BIT_ALARM_COLLIDE);
		}
	}
...
* Others:
***********************************************************/
void mma8451_process(void)
{
	static uint32_t counter_200ms=0;
	///侧翻报警操作
	if( tilt_status == TILT_ON )
	{
		if( rt_tick_get( ) - tilt_tick > 300 ) /*3秒*/
	...
	///定时读取加速度信息
	if(++counter_200ms%4 == 0)
	{
		rt_device_read(&dev_mma8451, 0, RT_NULL, 0);
	}

	///打印log信息到调试口
	if((mma845_print_style)&&(mma845_print_style<=3))
	{
		rt_kprintf( mma845_print_str[mma845_print_style-1] );
		mma845_print_style = 0;
	}
}
