...
* Return         : None
*******************************************************************************/
void SPI_Configuration_SPI2(void)
{
	...
	#if 0
	//PA2: SD卡插入检测（未插入悬空，插入拉低）PA脚内部上拉
	...
	SPI_InitStructure.SPI_NSS				= SPI_NSS_Soft;
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_256;          /* 42M/4=10.5M */
	SPI_InitStructure.SPI_FirstBit			= SPI_FirstBit_MSB;
	SPI_InitStructure.SPI_CRCPolynomial		= 7;
...
u8 SD_WaitReady(void)
{
    ...
    do
    {
        ...
        //if(retry==0xfffe)
        if(retry>400000)        //如果长时间写入没有完成，报错退出
        {
            return 1;
        }
    }while(r1!=0xFF);
...
u8 SD_GetCID(u8 *cid_data)
{
    ...
    r1 = SD_SendCommand_NoDeassert(CMD10, 0, 0xFF);
    if(r1 != 0x00)
    {
...
u8 SD_GetCSD(u8 *csd_data)
{
    ...
    r1 = SD_SendCommand_NoDeassert(CMD9, 0, 0xFF);
    if(r1 != 0x00)
    {
        return r1;  //没返回正确应答，则退出，报错
    }
...
u8 SD_WriteSingleBlock(u32 sector, const u8 *data)
{
    ...
    while(0xFF != SPI_ReadWriteByte(0xff))
    {
        retry++;
