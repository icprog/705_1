...
uint8_t		print_state = 0;		//0表示没有打印，1表示正在打印
...
static void printer_get_str_line( void )
{
	...
	while( fprinting == 0 )
	{
		if( rt_ringbuffer_getchar( &rb_printer_data, &CH ) == 0 )
		{
			print_state = 0;
			break;
		}
...
static void printer_get_str_line_old( void )
{
	unsigned char CH, CL;
