...
void tf_write( char *str )
{
	struct dirent *d;
	DIR *dirp;
	int fd, size; 
	char s[] = "RT-Thread Programmer!\n", buffer[512];
	struct dfs_filesystem rt_fs;
	struct dfs_fd rt_fd={"app.txt",FT_REGULAR,0,RT_NULL, DFS_O_RDWR};
	int  return_data;
	
	rt_sem_take( &sem_dataflash, RT_TICK_PER_SECOND * FLASH_SEM_DELAY );
	rt_sdcard_open( &dev_spi_sd,RT_DEVICE_FLAG_RDWR);
	#if 1
	memset(&rt_fs,0,sizeof(rt_fs));
	rt_fs.dev_id  = &dev_spi_sd;
	return_data = dfs_elm_mount(&rt_fs,0,0);
	if(return_data >= 0)
		{
		rt_fd.fs = &rt_fs;
		return_data = dfs_elm_open(&rt_fd);
		if(return_data >= 0)
			{
			memset(buffer,0,sizeof(buffer));
			return_data = dfs_elm_read(&rt_fd,buffer,sizeof(buffer));
			rt_kprintf("\r\n FILE LEN=%d",return_data);
			if(return_data > 0)
				{
				rt_kprintf("\r\n SD:%s\r\n DATA=",buffer);
				printf_hex_data(buffer,return_data);
				rt_kprintf("\r\n");
				}
			strcat(buffer,str);
			
			return_data = dfs_elm_write(&rt_fd,buffer,strlen(buffer));
			if(return_data > 0)
				{
				rt_kprintf("\r\n dfs_elm_write len =%d",return_data);
				}
			dfs_elm_close(&rt_fd);
			}
		}
	return_data = dfs_elm_unmount(&rt_fs);
	#endif
	#if 0
	dirp=opendir("/sd");
	if(dirp!= RT_NULL)
		...
		/*  ??/text.txt  ???,??????????????*/
		fd = open("/sd/text.txt", O_WRONLY | O_CREAT);
		if (fd >= 0)
			{
			write(fd, s, sizeof(s));
			close(fd);
		...
		/*  ??/text.txt  ???????*/
		fd = open("/sd/text.txt", O_RDONLY);
		if (fd >= 0)
			{
			size=read(fd, buffer, sizeof(buffer));
			close(fd);
			}
		rt_kprintf("读取到的内容2是:%s", buffer);
		}
	else
		{
		rt_kprintf("\r\n open sd card dir Error!");
		}
	#endif
...
void tf_open( void )
{
	...
	char s[] = "RT-Thread Programmer!\n", buffer[1024];
	struct dfs_filesystem rt_fs;
	struct dfs_fd rt_fd={"app.txt",0,0,RT_NULL, DFS_O_RDONLY};
	int  return_data;
