...
static enum _modem_style	modem_style=0;			///模块类型，0表示未知，1表示为M66,2表示为M50。
...
FINSH_FUNCTION_EXPORT( set_m66_ipr, control set_m66_ipr );
...
static void rt_thread_gsm_power_on( void* parameter )
{
	...
	*/
	...
	if(jt808_param.id_0xF00F == 0)
		return;
	sd_write_console("GSM模块POWER_ON!");
lbl_poweron_start:
	///先关掉电源
	GPIO_ResetBits( GSM_PWR_PORT, GSM_PWR_PIN );
	GPIO_ResetBits( GSM_TERMON_PORT, GSM_TERMON_PIN );
...
/*控制登网，还是断网*/
static void rt_thread_gsm_gprs_M66( void* parameter )
{
	char		buf[128];
	rt_err_t	err;
...
static void rt_thread_gsm_socket_M50( void* parameter )
{
	...
	if( pcurr_socket->state == CONNECT_CLOSING ) /*挂断连接*/
	{
		...
		err = gsm_send( buf, RT_NULL, rx_buf, RESP_TYPE_STR, RT_TICK_PER_SECOND * 10, 1 );
		if( err != RT_EOK )
		{
	...
	if( pcurr_socket->state == CONNECT_PEER )       /*建立连接*/
	{
		...
		err = gsm_send( buf, resp_linking_M50, RT_NULL, RESP_TYPE_FUNC, RT_TICK_PER_SECOND * 3, 10 );
		if( err != RT_EOK )
		{
			pcurr_socket->state		= CONNECT_ERROR;
			pcurr_socket->err_no	= 0x80 | CONNECT_PEER;
...
/*关于链路维护,只维护一个，多链路由上层处理*/
static void rt_thread_gsm_socket_M66( void* parameter )
{
	char		buf[128];
	rt_err_t	err;
	AT_CMD_RESP at_cmd_resp;
...
static void rt_thread_gsm_socket( void* parameter )
{
	...
	{
		rt_thread_gsm_socket_M66(parameter);
	}
}
