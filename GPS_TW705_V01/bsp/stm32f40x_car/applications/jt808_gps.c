...
static uint8_t	report_trans_normal = 0;	//0表示当前发送的是补报数据(发送命令0x0704)，1表示正常数据(命令0x0200)；
...
/*********************************************************************************
  ...
  *功能描述:查找最后一个写入的位置,  查找未上报记录最小id的位置, 
  			初始化数据存储相关参数，初始化GPS压缩数据数据存储相关参数(gps_pack_init)
...
void jt808_report_put( uint8_t* pinfo, uint16_t len )
{
	...
	{
		report_curr.addr	+= ( 12 + report_curr.len + 63 );	//加63是为了取一个比较大的值
		report_curr.addr	&= 0xFFFFFFC0;  /*指向下一个地址*/
	}else                                   /*还没有记录*/
	{
	...
	if(( gsm_socket[0].state == CONNECTED )&&(jt808_state == JT808_REPORT))  /*当前在线，保存并直接上报*/
	{
	...
	rt_sem_release( &sem_dataflash );
	sprintf(tempbuf,"写入GPS数据,ID=%05d,状态=%08X,报警=%08X,速度=%05d",head.id,jt808_status,jt808_param_bk.car_alarm,BYTESWAP2(gps_baseinfo.speed_10x));
...
/*批量查找记录*/
	...
	for( i = 0; i < ADDR_DF_POSTREPORT_SECT * 64; i++ )   /*每记录64字节,每sector(4096Byte)有64记录*/
	{
		sst25_read( addr, (uint8_t*)&head, sizeof( GPS_REPORT_HEAD ) );
		if( head.mn == 0x474E5353 )                     /*有效记录*/
		{
			...
			addr	+= ( 12 + head.len + 63 );	//加63是为了取一个比较大的值
