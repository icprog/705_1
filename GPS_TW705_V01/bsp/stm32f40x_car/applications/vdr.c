...
#define VDR_XX_START	ADDR_DF_VDR_BASE
...
type_vdr__sect_info sect_info[5] =
{
...
static void cb_tmr_200ms( void* parameter )
{
	...
	if(jt808_param_bk.car_run_time >= jt808_param.id_0x0057)
		{
 ...
 */
static uint32_t vdr_08_12_get_prevaddr( uint8_t vdr_id,uint16_t prev_num )
{
	uint16_t i,index,sector;
	uint8_t id = vdr_id - 8;
	
	index = sect_info[id].index;
	sector = sect_info[id].sector;
	
	for(i=0;i<prev_num;i++)
	{
		if(index == 0)
		{
			index = sect_info[id].record_per_sector - 1;
			if(sector == 0)
			{
				sector = sect_info[id].sector_count - 1;
			}
			else
			{
				sector--;
	...
	*/
	return sect_info[id].addr_start + sector * 4096 + index * sect_info[id].record_size;
}
...
/*********************************************************************************
  *函数名称:uint8_t vdr_11_get_1( MYTIME start, MYTIME end, uint8_t num, uint8_t *pout )
  *功能描述:读取疲劳驾驶记录，输入起止时间，获取指定的记录数，防止溢出
  *输	入:	start	:开始时间
  			end		:结束时间
  			num		:最大记录数量
  			pout	:保存获取到的信息，仅保存疲劳开始时间和结束时间，格式均为6字节BCD码，
  *输	出:	pout	:保存获取到的信息，仅保存疲劳开始时间和结束时间，格式均为6字节BCD码，
  *返 回 值:uint8_t	:获取到的超速记录数量
  *作	者:白养民
  *创建日期:2013-09-17
  *---------------------------------------------------------------------------------
  *修 改 人:
  ...
  *修改描述:
*********************************************************************************/
uint8_t vdr_11_get_1( MYTIME start, MYTIME end, uint8_t num, uint8_t *pout )
{
 	uint32_t	addr;
	uint8_t		buf[32];
	uint32_t	id;
	...
	for( i = 0; i < num+10; i++ )  /*不一次读进来了*/
	{
		addr = vdr_08_12_get_prevaddr(11,i);
		...
		mytime1 = mytime_from_bcd(buf + 4);
		mytime2 = mytime_from_bcd(buf + 10);
		if(((mytime1>=start)&&(mytime1<=end))||((mytime2>=start)&&(mytime2<=end)))
		{
			memcpy( pout + count * 12, buf + 4, 6 );
			memcpy( pout + count * 12 + 6, buf + 10, 6 );
			count++;
			if(count >= num)
				break;
		}
...
/*********************************************************************************
  *函数名称:void vdr_11_put( void )
  *功能描述:写入疲劳驾驶记录到flash中
			   超时,疲劳驾驶记录
			   有可能超时了，还在驾驶，这时要动态生成
...
void vdr_11_put( void )
{
	...
	mytime_to_bcd( buf + 22, utc_to_mytime(jt808_param_bk.utc_over_time_start));
	mytime_to_bcd( buf + 28, utc_to_mytime(jt808_param_bk.utc_over_time_end) );

	PACK_INT( buf + 34, ( jt808_param_bk.vdr11_longi_start * 6 / 10 ) );	///郭爷这里有错误(vdr11_longi_start*6)，应该是(vdr11_longi_start*6/10)
	PACK_INT( buf + 38, ( jt808_param_bk.vdr11_lati_start * 6 / 10 ) );
	PACK_WORD( buf + 42, jt808_param_bk.vdr11_alti_start );

	PACK_INT( buf + 44, ( jt808_param_bk.vdr11_longi_end * 6 / 10) );
	PACK_INT( buf + 48, ( jt808_param_bk.vdr11_lati_end * 6 / 10 ) );
	PACK_WORD( buf + 52, jt808_param_bk.vdr11_alti_end);
 ...
 */
/*********************************************************************************
  *函数名称:uint8_t vdr_16_get( MYTIME start, MYTIME end, uint8_t num, uint8_t *pout )
  *功能描述:读取超速驾驶记录，输入起止时间，获取指定的记录数，防止溢出
  *输	入:	start	:开始时间
  			end		:结束时间
  			num		:最大记录数量
  			pout	:保存获取到的超速信息
...
uint8_t vdr_16_get( MYTIME start, MYTIME end, uint8_t num, uint8_t *pout )
{
	...
	if(vdr16_addr_curr < VDR_16_START)
		vdr16_addr_curr = VDR_16_START;
	vdr16_addr_curr &= 0xFFFFFFE0;
	...
	for( i = 0; i < 256; i++ )  /*不一次读进来了*/
	{
		...
		if(((mytime1>=start)&&(mytime1<=end))||((mytime2>=start)&&(mytime2<=end)))
		{
			...
			memcpy( pout + count * 14 + 6, buf + 10, 6 );
...
/*********************************************************************************
  *函数名称:void vdr_16_put( MYTIME start, MYTIME end, uint16_t speed_min, uint16_t speed_max, uint16_t speed_avg )
  *功能描述:写入超速驾驶记录到flash中
  *输	入:	start	:开始时间
  			end		:结束时间
  			speed_min:最小速度
  			speed_max:最大速度
		...
		if( utc_overspeed_start )                                           /*判断超速持续事件*/
		{
			if( utc_now - utc_overspeed_start >= jt808_param.id_0x0056 )    /*已经持续超速*/
			{
				...
				if( gps_speed > overspeed_max )
				{
					...
					jt808_param_bk.over_speed_max= gps_speed;
				}
...
MYTIME vdr_08_12_init( uint8_t vdr_id, uint8_t format )
{
	...
	rt_kprintf( "\n%d>vdr%02d time=%08x sect=%03d index=%03d", rt_tick_get( ), vdr_id, mytime_ret, sect_info[id].sector, sect_info[id].index );
	return mytime_ret;
}
...
uint8_t vdr_08_12_fill_data( JT808_TX_NODEDATA *pnodedata )
{
	...
	rt_sem_take( &sem_dataflash, RT_TICK_PER_SECOND * FLASH_SEM_DELAY_2 );  /*准备获取数据*/
...
/*********************************************************************************
  *函数名称:void vdr_data_init(void)
  *功能描述:初始化行驶记录仪(VDR)数据存储相关参数
...
void vdr_rx_8700( uint8_t * pmsg )
{
	...
	switch( cmd )
	{
		...
		case 0x03:
			...
			i			= (jt808_param.id_0xF032 & 0xFFFF) * 10;       /*初次安装里程 0.1KM BCD码 00-99999999*/
			...
			i			= jt808_param_bk.car_mileage/ 3600;       /*总里程 0.1KM BCD码 00-99999999*/
			tmpbuf[0]	= ( ( ( i / 10000000 ) % 10 ) << 4 ) | ( ( i / 1000000 ) % 10 );
			tmpbuf[1]	= ( ( ( i / 100000 ) % 10 ) << 4 ) | ( ( i / 10000 ) % 10 );
			tmpbuf[2]	= ( ( ( i / 1000 ) % 10 ) << 4 ) | ( ( i / 100 ) % 10 );
			tmpbuf[3]	= ( ( ( i / 10 ) % 10 ) << 4 ) | ( i % 10 );
		...
		case 0x04: /*特征系数*/
			...
			vdr_pack_buf( buf + 9, gps_baseinfo.datetime, 6, &fcs );
			vdr_pack_word( buf + 15, jt808_param.id_0xF032 & 0xFFFF, &fcs );
			buf[17] = fcs;
