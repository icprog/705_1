...
/*********************************************************************************
  ...
  *修改描述:
*********************************************************************************/
void area_jt808_commit_ack( u16 fram_num, u16 cmd_id, u8 statue )
{
	u8 pbuf[8];
...
rt_err_t area_jt808_del( uint8_t linkno, uint8_t *pmsg, ENUM_AREA del_State )
{
	...
	jt808_tx_0x0001( fram_num, cmd_id, 0 );
	return RT_EOK;
}
...
rt_err_t area_jt808_0x8600( uint8_t linkno, uint8_t *pmsg )
{
	...
	jt808_tx_0x0001( fram_num, 0x8600, 0 );
	return RT_EOK;
}
...
// u16 msg_len;
	...
	jt808_tx_0x0001( fram_num, 0x8602, 0 );
	return RT_EOK;
}
...
// u16 msg_len;
	...
	if( pTempHead )
	{
		...
		memcpy( (void*)&pTempHead->ID, msg, datalen );
		area_flash_write_area( area_buffer, sizeof( area_buffer ), pTempHead );
		jt808_tx_0x0001( fram_num, 0x8604, 0 );
		rt_free( pTempHead );
	}else
	{
		jt808_tx_0x0001( fram_num, 0x8604, 3 );
	}
	return RT_EOK;
}
...
rt_err_t area_jt808_0x8606( uint8_t linkno, uint8_t *pmsg )
{
	...
	if( ( pmsg[2] & 0x20 ) == 0 )
	{
		...
		if( tempu16data )
		{
			jt808_tx_0x0001( fram_num, 0x8606, 3 );
			return RT_ERROR;
		}
		pmsg += 16;
	}else
	...
	if( pTempHead )
	{
		...
		jt808_tx_0x0001( fram_num, 0x8606, 0 );
		rt_free( pTempHead );
	}else
	{
		jt808_tx_0x0001( fram_num, 0x8604, 3 );
...
void area_clear_alarm( uint32_t clear_bits )
{
 ...
 for(i=0;i<AREA_ALARM_NUM;i++)
 	...
 	if(BIT(area_alarm_array[i].alarm_bit & 0x1F) & clear_bits)
 		...
 		if( area_alarm_array[i].alarm_bit == 22)
 			{
...
AREA_IN:     ///进区域
	...
	if( road_id != AreaInfo->road_id )                                         	///行驶路段发生改变
	{
		...
		{
			area_alarm_road_timer( ( utc_now - AreaInfo->road_in_utc ), AreaInfo->road_id, 0 );
		}else if( utc_now - AreaInfo->road_in_utc > AreaInfo->road_timer_max )  ///路段行驶过长
		{
			area_alarm_road_timer( ( utc_now - AreaInfo->road_in_utc ), AreaInfo->road_id, 1 );
