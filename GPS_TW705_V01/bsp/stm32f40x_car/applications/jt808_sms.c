...
#define BITMASK_7BITS 0x7F
	...
	*pDst = 0;
	...
	sms_tx( buf, ( strlen( buf ) - 2 ) >> 1 );				//因为CMGS发送的长度不包括SCA的长度，在该发送短信中SCA为"00"，所以减2
}
...
//	strcpy(buf,smsc,strlen(smsc));
	strcpy( buf, "001100" );	//不设置短信中心方式发送，默认均采用该方法发送
	count = strlen( DA_buf );
	strcat( buf, DA_buf );
	strcat( buf, "00088F" );	//从左往右含义，第一个00是PID，默认均为00；08为DCS表示UCS2编码；8F为VP表示消息有效期 (8F+1)*5 分钟。
//	ucs2bit编码
	pSrc	= info;
	pDst	= buf + ( count + 12 + 2 ); //sender长度+12ASC+2预留TP长度
	while( *pSrc )
