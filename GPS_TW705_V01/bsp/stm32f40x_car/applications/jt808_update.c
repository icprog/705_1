...
rt_err_t updata_jt808_0x8108( uint8_t linkno, uint8_t *pmsg )
{
	...
	if( ( pmsg[2] & 0x20 ) == 0 )
	{
		...
		jt808_tx_0x0001(fram_num, 0x8108, 1);
		goto UPDATA_END;
	}
	...
	if( cur_package_num == 1 )                              ///第一包
	{
		...
		if(memcmp(jt808_param.id_0xF010,soft_ver,Tempu32data ))
			{
			memcpy(jt808_param.id_0xF010,soft_ver,Tempu32data );
			param_save( 0 );
			new_files = 1;
			}
...
UPDATA_OK:
	jt808_tx_0x0001(fram_num, 0x8108, 0);
...
void update_send_info(void)
{
	...
	rt_kprintf("\n 固件版本:%s",tempbuf+74);
	rt_kprintf("\n 运 营 商:%s",tempbuf+94);
	rt_kprintf("\n 开始地址:");printf_hex_data(tempbuf+110, 4);
	rt_kprintf("\n 软件版本:");printf_hex_data(tempbuf+104, 6);
	rt_kprintf("\n ");
