...
typedef __packed struct _jt808_param
{
	...
	uint8_t     id_0xF00F;		//0xF00F 车辆信息设置是否完成,	0:未设置     1:设置完成
...
/*打印相关*/
	...
	char		id_0xF049[64];  /*0xF049 备份服务器地址2*/
	uint16_t	id_0xF04A;      /*0xF04A 备份TCP端口2*/
	char		id_0xF04B[64];  /*0xF04B 备份务器地址3*/
	uint16_t	id_0xF04C;      /*0xF04C 备份TCP端口3*/
}JT808_PARAM;
...
#define BKSRAM_VERSION				0x00000001
#define BKSRAM_FORMAT				0x57584448


typedef __packed struct
{
	uint16_t			get_speed;				///根据脉速表计算出的速度,单位为KM/H
	uint32_t			get_speed_tick;			///脉速表上次获取到脉冲的时刻，单位为tick
