...
static void display( void )
{
	...
	lcd_text12( 50, 4, car_info.chepai, strlen( car_info.chepai ), LCD_MODE_SET );
	...
	if( mode==SELECT ) /*选择状态*/
	{
		if( fInvert )
		{
			...
			if( modify_pos == 0 )
			{
				lcd_text12( 50, 4, car_info.chepai, 2, LCD_MODE_INVERT );
			}else
			{
				lcd_text12( 50 + modify_pos * 6 + 6, 4, car_info.chepai + modify_pos + 1, 1, LCD_MODE_INVERT );
			}
		}
	...
	switch( key )
	{
		...
		case KEY_OK:
			...
			}else if( mode == INPUT )
			{
				if( input_pos == 0 )
				{
					memcpy( car_info.chepai, hz + pos * 2, 2 );
				}else
				{
					car_info.chepai[input_pos + 1] = *( asc + pos );
					car_info.chepai[input_pos + 2] = 0;
				}
				input_pos++;
				pos = 0;
			}else				/*编辑状态*/
			{
				...
				{
					car_info.chepai[modify_pos + 1] = *( asc + pos );
				}
				mode = SELECT;
			}
			display( );
