...
static void display( void )
{
	...
	}else
	{
		if(seting_info == 0)
		{
			...
			if(info->st)
			{
				lcd_text12( 96, 4, "点播", 4, 3 - ( pos & 0x01 ) * 2 );         /*SET=1 INVERT=3*/
			}
			else
			{
				lcd_text12( 96, 4, "取消", 4, 3 - ( pos & 0x01 ) * 2 );         /*SET=1 INVERT=3*/
			}
			if( ( index + 1 ) < count )
			{
				...
				if(info->st)
				{
					lcd_text12( 96, 18, "点播", 4, ( pos & 0x01 ) * 2 + 1 );         /*SET=1 INVERT=3*/
				}
				else
				{
					lcd_text12( 96, 18, "取消", 4, ( pos & 0x01 ) * 2 + 1 );         /*SET=1 INVERT=3*/
				}
			}
		}
		else
		{
			...
			lcd_text12( 16, 18, "点播", 4, ( info_is_set & 0x01 ) * 2 + 1 );         /*SET=1 INVERT=3*/
			lcd_text12( 80, 18, "取消", 4, 3 - ( info_is_set & 0x01 ) * 2  );         /*SET=1 INVERT=3*/
		}
	}
	lcd_update_all( );
}
